...
This file is a flat array, indexed by page VA, of per-page entries
generated by the kernel. It does not expose actual hardware PTEs.
It also does not expose the addresses of the page table itself.

Since 4.2 this requires CAP_SYS_ADMIN. Users without this capability
may see the simulated-PTE as zeroes.

There is a mapping per userspace process address space. The kernel might
not be mapped into this address space.
...
class PAMap:
    ...
    def entry(self, va):
        ...
        if ebs is not None:
            return KernelPME(ebs, pagemap=self)
        else:
            return None
...
def scan_pid(pid):
    ...
    for (ln, vaddr, vaend) in areas:
        ...
        while vaddr < vaend:
            ...
            vaddr += m.page_size
    if n_va_areas == 0:
        print("Selected target has no VA mappings", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("address", nargs='?', type=auto_int, default=0, help="base address of virtual memory range")
    parser.add_argument("--size", type=auto_int, default=0, help="size of virtual memory range")
