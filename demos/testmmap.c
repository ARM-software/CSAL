...
void show_mem(void const *vav, unsigned long size, unsigned long phys_addr)
{
  ...
  for (i = 0; i < size && i < 4096; i += 32) {
    int j;
    printf("V:0x%lx P:0x%lx ", (unsigned long)(va + i), phys_addr + i);
    for (j = 0; j < 32; ++j) {
      printf(" %02x", va[i+j]);
    }
    printf("\n");
